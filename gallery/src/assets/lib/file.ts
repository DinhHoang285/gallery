...
export const nextImageLoader = ({ src, width, quality }: NextImageLoaderProps): string =>
  `${src}?w=${width}&q=${quality || 75}`;

export function beforeUploadImage(file: File): boolean {
...
export const isMediaType = (file: IFile, typeCompart: 'all' | 'image' | 'video' = 'all') => {
  const imageTypes = ['photo', 'image', 'gif'];
